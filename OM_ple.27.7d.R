### ------------------------------------------------------------------------ ###
### create OM for eastern English Channel plaice ple.27.7d ####
### ------------------------------------------------------------------------ ###
...
refpts <- list(
  ...
  EqSim_Btrigger = 34942, EqSim_Fmsy = 0.203, EqSim_Fpa = 0.246, 
  EqSim_Bpa = 34942, EqSim_Blim = 25105,
  ### real OM MSY values
  Fmsy = NA, Bmsy = NA, Cmsy = NA, Blim = NA, 
  Blim_RR0 = NA, Blim_BB0 = NA)
# round(min(ssbtable(fit)[, "Estimate"]))

### intermediate year catch advice
int_yr_catch <- 1151 ### from 2025 advice sheet
...
### benchmarked in 2025
### reference points presented/updated at WGNSSK 2025
### bio/sel year: 2020-2024 - last 5 years
### rec years: 1980-2023 - all excluding last

### M/maturity age dependend, time invariant

### baseline OM
# debugonce(create_OM)
### SRR: segmented regression with breakpoint fixed to Blim = 25110
create_OM(stk_data = stk_data, idx_data = idx_data, SAM_conf = SAM_conf,
          n = 1000, n_years = 100,
          yr_data = 2024, 
          disc_survival_OM = 0, disc_survival_MP = 0,
          int_yr_add = TRUE, 
          int_yr_catch = int_yr_catch, int_yr_catch_split = TRUE,
          n_sample_yrs = 5, sr_model = "segreg", sr_parallel = 10,
          sr_ar_check = TRUE, sr_fixed = list(b = 25110),
          idxB = FALSE, idxL = FALSE, PA_status = FALSE,
          refpts = refpts, stock_id = "ple.27.7d", OM = "baseline")
